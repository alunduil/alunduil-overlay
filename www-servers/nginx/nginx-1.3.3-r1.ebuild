...
# $Header: /var/cvsroot/gentoo-x86/www-servers/nginx/nginx-1.3.3.ebuild,v 1.3 2012/07/30 08:18:38 hollow Exp $
...
# http_passenger (www-apache/passenger)
HTTP_PASSENGER_MODULE_BD="nginx_http_passenger_module"
HTTP_PASSENGER_MODULE_SD="${S}/${HTTP_PASSENGER_MODULE_BD}"
HTTP_PASSENGER_MODULE_WD="${WORKDIR}/${HTTP_PASSENGER_MODULE_BD}"
...
src_prepare() {
	...
	cp -r dirname $(qlist passenger | grep 'ext/nginx' | head -n 1) \
	${HTTP_PASSENGER_MODULE_SD}
}
...
src_configure() {
	...
	if use nginx_modules_http_passenger; then
		http_enabled=1
		mycont+=" --add-module=${HTTP_PASSENGER_MODULE_WD}"
	fi
