...
src_install() {
	...
	doins config/providers/mysql-lvm.conf || die "Could not insert mysql-lvm
	configuration!"

	insinto /etc/holland/backupsets/examples
	doins config/backupsets/examples/mysql-lvm.conf || die "Could not insert
	mysql-lvm backupset example!"
}
